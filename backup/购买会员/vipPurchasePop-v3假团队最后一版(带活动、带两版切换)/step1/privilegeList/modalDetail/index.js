...
*
* 2019 沿用遗留代码，新皮未重构
* 2020.05.06 - class to hook
*
*  */

const INTRODUCE_DETAIL = [
  {
    type: 'matting',
    h: '自动抠图功能',
    p: [
...
const ModalDetail = React.memo(({ type }) => {
  const [curIndex, setCurIndex] = useState(TYPE_LIST.indexOf(type));
  const prevIntroduce = () => setCurIndex(curIndex - 1);
  const nextIntroduce = () => setCurIndex(curIndex + 1);

  const detail = INTRODUCE_DETAIL[curIndex];

  return (
    <div styleName="profession-introduce">
      { curIndex !== 0 ? (
        <i styleName="prev-btn" onClick={prevIntroduce} />
      ) : null }
      { curIndex !== INTRODUCE_DETAIL.length - 1 ? (
        <i styleName="next-btn" onClick={nextIntroduce} />
      ) : null }
